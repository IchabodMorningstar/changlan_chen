...
- Focused on determining neural network structures required to generate human-like behaviors in natural environments, such as attention modules and pre-trained CNNs to replicate the naturalistic constraints;
- Combined the pre-trained CNNs with RNNs to simulate the visual capacity limits and orientational biases, and then analyze these constraints;
...
- Pre-processed oculomotor delayed-response (ODR) and Match Non-match (MNM) datasets of money neural activity recordings from timestamps, marking spiking data to "window data" which analyzed the number of impulses within a certain window time;
- Designed a data processing pipeline that integrated deep learning frameworks with traditional statistical methods;
- Built LeakyRNN model, and trained it on synthetic data to simulate the recorded data from the ODR test of neural activity in monkey brains;
- Compared the simulations of different DNN models, including a two-layer FNN, a simple CNN, and an LSTM, for the Match/Non-Match dataset of spatial and feature information, and achieved over 98\% accuracy in predicting memory-specific neural activity.

### Machine Learning Security Attacks and Defenses for Connected Autonomous Vehicles
...
- Improved the robustness of the Activation Clustering (AC) backdoor detection algorithm and proposed backdoor defense method ViT4Purge (Vision Transformer for Purge), which outperformed AC and Scale-up.
...
2. Hanif, M., Luoga, L., Chen, C., Jabari, S., Ouni, B. and Shafique M., 2024. ViT4Purge: Detecting Backdoor Attacks by Exploiting the Attention Capabilities of ViTs. Submitted to Design Automation Conference, Submitted.
